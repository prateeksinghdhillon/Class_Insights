...
export const savePassword = async (event) => {
  try {
    ...
    if (event.query.tag === GLOBAL_CONSTANT.OTP_TAG.REGISTER) {
    ...
    } else if (event.query.tag === GLOBAL_CONSTANT.OTP_TAG.FORGET_PASSWORD) {
      let userRes = await UserModel(event.query.schoolId).findOneAndUpdate(
        {
